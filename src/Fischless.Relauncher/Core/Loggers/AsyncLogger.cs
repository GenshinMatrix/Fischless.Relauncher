...
public sealed class AsyncLogger : ILogger, IDisposable
{
    ...
    internal readonly ConcurrentQueue<LoggerMessage> Queue = new();
    ...
    public AsyncLogger()
    {
        ...
        _ = Task.Factory.StartNew(() =>
        {
            while (IsEnabled)
            {
                ...
                while (Queue.Count > 0 && Queue.TryDequeue(out LoggerMessage msg))
                {
                    if (IsEnabled)
                    {
                        lock (WriterLock)
    ...
    private void Log(string type, string message)
    {
        ...
        if (IsEnabled)
        {
            Queue.Enqueue(new LoggerMessage(sb.ToString()));
            ResetEvent.Set();
        }
    }
